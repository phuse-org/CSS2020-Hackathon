...
adjusted_pcts <- counts_and_weight %>% 
  ...
  summarize(adj_pct_n = sum(weighted_pct_by_study)) %>% 
...
ht <- huxtable::as_hux(final, add_colnames=FALSE) %>%
  huxtable::set_bold(1:2, 1:ncol(final), TRUE) %>% # bold the first row
  huxtable::set_align(1:2, 1:ncol(final), 'center') %>% # Center align the first row 
  huxtable::set_align(3:nrow(final), 3:ncol(final), 'center') %>% # Center align the results
  huxtable::set_valign(1:2, 1:ncol(final), 'bottom') %>% # Bottom align the first row
  huxtable::set_bottom_border(2, 1:ncol(final), 1) %>% # Put a border under the first row
  huxtable::set_width(1) %>% # Set the table width
  huxtable::set_col_width(c(.3, rep(.7/6, 6))) # Set the column widths
ht
```
